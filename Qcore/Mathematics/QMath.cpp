...
template<> const float Math<float>::MIN_REAL(FLT_MIN);
...
template<> const double Math<double>::MIN_REAL(DBL_MIN);
...
double Math<double>::FastInvSqrt (double dValue)
{
    ...
    Int64 i = *(Int64*)&dValue;
#if defined(Q_USING_VC80) || defined(Q_USING_VC70) || defined(Q_USING_VC6)
    i = 0x5fe6ec85e7de30da - i/2;
#else
    i = 0x5fe6ec85e7de30daLL - i/2;
#endif
    dValue = *(double*)&i;
